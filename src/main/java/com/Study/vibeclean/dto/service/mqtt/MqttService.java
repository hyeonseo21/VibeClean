...
public class MqttService {
    ...
    public void init() throws MqttException {
        ...
        // STM32 → 서버: ai-toppic 구독
        mqttClient.subscribe(aiTopic, (topic, message) -> {
            String payload = new String(message.getPayload(), StandardCharsets.UTF_8);
            log.debug("Received MQTT: topic={}, payload={}", topic, payload); // 어떤 토픽에서 구독받은 건지 그리고 내용 출력
            aiHandler.AiTelemetry(payload); // 받은 내용을 핸들러로 넘겨서 DB에 저장한다.
        });

        // STM32 → 서버: auto2D 구독
        mqttClient.subscribe(auto2DTopic, (topic, message) -> {
            String payload = new String(message.getPayload(), StandardCharsets.UTF_8);
            log.debug("Received MQTT: topic={}, payload={}", topic, payload); // 어떤 토픽에서 구독받은 건지 그리고 내용 출력
